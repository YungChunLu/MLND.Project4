    ...
    def step(self):
        ...
        if self.primary_agent is not None:
            if self.enforce_deadline and self.agent_states[self.primary_agent]['deadline'] <= 0:
                self.done = True
                self.primary_agent.records.append(self.primary_agent.penalties)
                self.primary_agent.rewards.append(self.primary_agent.reward)
    ...
    def act(self, agent, action):
        ...
        if action is not None:
            if move_okay:
                ...
                if agent is self.primary_agent:
                    self.primary_agent.penalties += 1
        ...
        if agent is self.primary_agent:
            if state['location'] == state['destination']:
                ...
                self.done = True
                agent.success += 1
                print "# of Success: {}".format(agent.success)
                agent.records.append(agent.penalties)
